...
def contrastive_loss(h1, h2, tau=0.4):
    ...
    loss = -0.3 * (torch.log(numerator / denominator).mean())
    return loss
    ...
    for f in range(1, args.cv_num + 1):
        ...
        def train(epochs):
            ...
            if epoch != epochs - 1:
                ...
                loss_v = 0.7*(loss_k + F.binary_cross_entropy_with_logits(re1.t(), MA, pos_weight=pos_weight) + F.binary_cross_entropy_with_logits(ret.t(), MA, pos_weight=pos_weight))
                loss_r_h = F.binary_cross_entropy_with_logits(reMMDD.t(), MA, pos_weight=pos_weight) + F.binary_crossntropy_with_logits(reMD.t(), MA, pos_weight=pos_weight) + F.binary_cross_entropy_with_logits(rec.t(), MA, pos_weight=pos_weight)

                loss = loss_r_h + loss_v + loss_c
